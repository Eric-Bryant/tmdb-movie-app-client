<template>
  <v-slide-group>
    <v-slide-item v-for="item in info" :key="item.id">
      <v-card :max-width="mobile ? '100px' : '185px'" class="ma-2">
        <router-link :to="url(item)" exact>
          <v-img
            ...
            eager
            :src="image(item)"
            :height="mobile ? '115px' : '250px'"
          ></v-img>
        </router-link>
        <v-card-title class="info-title">{{ title(item) }}</v-card-title>
      </v-card>
    </v-slide-item>
  </v-slide-group>
</template>
